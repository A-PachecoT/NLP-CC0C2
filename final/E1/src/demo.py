#!/usr/bin/env python3
"""Demo E1: Multi-Agent QA - Modos verbose, stream y show-logs."""
import json
import sys
...
def print_header():
    print(f"\n{C.BOLD}{'='*60}")
    print("  MULTI-AGENT QA SYSTEM - E1")
    print("  LangGraph + RAG + GPT-4.1-mini")
    print(f"{'='*60}{C.END}")
...
def print_verbose_step(node_name: str, log: dict):
    ...
    if node_name == "manager":
        ...
        if answers:
            ...
            text = answers[0][:200] + "..." if len(answers[0]) > 200 else answers[0]
...
def run_verbose(question: str):
    ...
    result = run_query(question, save_logs=True)
